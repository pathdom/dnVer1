...
async function getUser(role, id) {
  if (role === 'admin') {
    const [rows] = await db.query('SELECT id, username, ho_ten, avatar_url FROM admin WHERE id = ?', [id]);
    const a = rows[0];
    if (!a) return null;
  ...
  if (role === 'staff') {
    const [rows] = await db.query('SELECT id, ho_ten, chuc_danh, bo_phan, avatar_url FROM nhan_vien WHERE id = ?', [id]);
    const s = rows[0];
    if (!s) return null;
    return { role: 'staff', id: s.id, name: s.ho_ten, roleLabel: s.chuc_danh || s.bo_phan || 'Nhân viên', initials: initialsOf(s.ho_ten, 'NV'), avatarUrl: s.avatar_url || null };
  }
  return null;
}

async function listPeople(excludeRole, excludeId) {
  const [admins] = await db.query('SELECT id, username, ho_ten, avatar_url FROM admin');
  const [staffs] = await db.query("SELECT id, ho_ten, chuc_danh, bo_phan, avatar_url FROM nhan_vien WHERE trang_thai = 'Đang làm việc' OR trang_thai IS NULL");
  const people = [
    ...admins.map(a => ({
    ...
    ...staffs.map(s => ({
      ...
      department: s.bo_phan || 'Khác', initials: initialsOf(s.ho_ten, 'NV'), avatarUrl: s.avatar_url || null
    }))
  ];
