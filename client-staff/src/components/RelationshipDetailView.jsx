...
function getStampClass(statusText) {
  switch (statusText) {
    case 'Mới tiếp nhận': return 'stamp stamp-teal';
    case 'Đang tư vấn': return 'stamp stamp-gold';
    case 'Tiềm năng cao': return 'stamp stamp-green';
    case 'Đã chốt': return 'stamp stamp-blue';
    case 'Hoạt động': return 'stamp stamp-green';
    case 'Hủy':
    case 'Tạm ngưng': return 'stamp stamp-gray';
    default: return 'stamp stamp-teal';
  }
}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '13px', marginBottom: '14px' }}>
            {MOCK_DOCS.map((doc, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', background: 'var(--bg)', borderRadius: '8px', border: '1px solid var(--border)' }}>
                ...
                <span className={doc.status === 'Đã nhận' ? 'stamp stamp-green' : 'stamp stamp-gold'} style={{ fontSize: '10px' }}>{doc.status}</span>
              </div>
            ))}
          </div>
          <button
